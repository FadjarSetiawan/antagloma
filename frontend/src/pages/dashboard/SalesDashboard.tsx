...
export const SalesDashboard: React.FC = () => {
  ...
  const qc = useQueryClient(); const [detail, setDetail] = useState<Order | null>(null); const [photos, setPhotos] = useState<OrderPackage | null>(null); const [trackingOrder, setTrackingOrder] = useState<Order | null>(null); const [confirm, setConfirm] = useState<Order | null>(null); const [error, setError] = useState('');
  ...
  return <div className="space-y-4 sm:space-y-5 max-w-5xl pb-24 font-sans text-slate-900 px-1 sm:px-0">
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          <span className="text-sm sm:text-base font-extrabold text-white block">Pesanan Dibuat Hari Ini · {todayCount} Order</span>
    ...
    <section aria-label="Ringkasan status pesanan" className="grid grid-cols-2 gap-2.5 sm:gap-3.5">{summaryCards.map(card => <article key={card.title} className="bg-white border border-slate-200/90 rounded-2xl p-4 flex flex-col justify-between space-y-3 shadow-2xs hover:border-[#04593f] hover:shadow-xs transition-all group"><div className="flex items-start justify-between gap-2"><span className="w-8 h-8 rounded-xl bg-emerald-50 text-[#04593f] flex items-center justify-center shrink-0">{card.icon}</span><strong className="text-xl sm:text-2xl leading-none text-slate-950">{card.count}</strong></div><div><h2 className="text-xs sm:text-sm font-bold text-slate-800 leading-tight block">{card.title}</h2><p className="mt-1 flex items-center text-xs text-slate-500 leading-none truncate">{card.detail}</p></div><button onClick={card.onClick} className="w-full py-2 px-2.5 bg-emerald-50 group-hover:bg-[#04593f] text-[#04593f] group-hover:text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1 transition-all cursor-pointer shadow-2xs"><span>{card.action}</span><span aria-hidden="true">›</span></button></article>)}</section>
    ...
    {location.hash === '#riwayat-pesanan' && <section id="riwayat-pesanan"><h2 className="text-sm font-bold mb-2">Riwayat Pesanan</h2><div className="space-y-2">{history.length === 0 ? <Empty text="Belum ada pesanan di riwayat." /> : history.map(o => <HistoryOrderCard key={o.id} order={o} onPhoto={setPhotos} onTracking={setTrackingOrder} />)}</div></section>}
...
const HistoryOrderCard: React.FC<{ order: Order; onPhoto: (pkg: OrderPackage) => void; onTracking: (order: Order) => void }> = ({ order, onPhoto, onTracking }) => <article className="h-full bg-white border border-slate-200/90 rounded-2xl p-3.5 space-y-3 shadow-2xs"><div><b className="text-xs">{order.order_number}</b><p className="font-bold text-sm mt-1">{order.customer_name}</p></div>{order.packages?.length ? <div className="space-y-2">{order.packages.map(p => <div key={p.id} className="rounded-lg bg-slate-50 p-3 space-y-2"><div className="flex items-center justify-between gap-2"><b className="text-xs">{packageLabel(order, p)}</b>{hasPackagePhoto(p) && <button onClick={() => onPhoto(p)} className="min-h-11 shrink-0 text-xs font-bold text-[#04593f]">Lihat Foto Paket</button>}</div><div className="text-xs text-slate-600 space-y-0.5">{p.items?.map(item => <p key={item.order_item_id}>• {item.product_name || 'Tanaman'} ×{item.quantity}</p>)}</div><p className="text-xs text-slate-600">{p.tracking_number ? `✓ ${p.tracking_number}` : '○ Belum Resi'}</p></div>)}</div> : <div><p className="text-[10px] font-bold uppercase text-slate-400">Pesanan</p><ul className="mt-1 text-xs text-slate-600 space-y-0.5">{order.items?.map(item => <li key={item.id}>• {item.product_name} ×{item.quantity}</li>)}</ul></div>}<div className="flex items-center justify-between gap-2"><div><p className="text-xs text-emerald-700 font-bold">✓ Selesai Diinfokan</p><p className="text-[10px] text-slate-400">{order.sales_informed_at}</p></div>{(order.packages || []).some(p => p.tracking_number?.trim()) || order.tracking_number?.trim() ? <button onClick={() => onTracking(order)} className="min-h-10 shrink-0 rounded-xl bg-slate-700 px-3 text-xs font-bold text-white">Lihat Resi</button> : null}</div></article>;
const TrackingViewModal: React.FC<{ order: Order; onClose: () => void }> = ({ order, onClose }) => <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/55 p-4" onClick={onClose}><div className="w-full max-w-md rounded-3xl bg-white p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}><div className="mb-4 flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-emerald-700">Resi Pengiriman</p><h3 className="mt-0.5 text-base font-extrabold text-slate-900">{order.order_number}</h3></div><button type="button" onClick={onClose} className="min-h-10 rounded-xl bg-slate-100 px-3 text-xs font-bold text-slate-600">Tutup</button></div><div className="space-y-2.5">{order.packages?.length ? order.packages.map(pkg => <div key={pkg.id} className="rounded-2xl border border-emerald-100 bg-emerald-50/60 px-4 py-3"><div className="flex items-center justify-between gap-3"><span className="text-xs font-bold text-slate-700">Paket {pkg.letter}</span><span className="text-sm font-extrabold tracking-wide text-emerald-800">{pkg.tracking_number || '-'}</span></div>{pkg.shipping_cost != null && <p className="mt-1 text-xs text-slate-500">Ongkir: Rp{Number(pkg.shipping_cost).toLocaleString('id-ID')}</p>}</div>) : <div className="rounded-2xl border border-emerald-100 bg-emerald-50/60 px-4 py-3"><p className="text-sm font-extrabold tracking-wide text-emerald-800">{order.tracking_number || '-'}</p>{order.shipping_cost != null && <p className="mt-1 text-xs text-slate-500">Ongkir: Rp{Number(order.shipping_cost).toLocaleString('id-ID')}</p>}</div>}</div></div></div>;
