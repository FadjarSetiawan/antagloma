  ...
  return <div className="fixed inset-0 z-[10050] flex items-center justify-center bg-slate-950/55 p-3" onClick={onClose}>
    <div className="max-h-[94vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white shadow-2xl" onClick={e => e.stopPropagation()}>
      ...
      <div className="space-y-5 p-5"><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="flex justify-between gap-3"><div><p className="font-extrabold text-slate-900">{order.order_number}</p><p className="text-sm text-slate-600">{order.customer_name} · {order.phone}</p><p className="text-sm text-slate-600">{order.items?.reduce((n, i) => n + i.quantity, 0) || 0} tanaman · Rp {(order.plant_total || 0).toLocaleString('id-ID')}</p></div><span className="rounded-lg bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-800">Selesai</span></div></div>
        <section><label className="mb-2 block text-sm font-bold text-slate-800">Pilih paket yang diretur <span className="text-rose-600">*</span></label><div className="space-y-2">{packages.map(pkg => { const checked = selected.includes(pkg.id); const packageItems = (pkg.items || []).map(allocation => { const item = order.items?.find(i => i.id === allocation.order_item_id); return { id: item?.tree_code || item?.product_name || allocation.product_name || '-', grade: item?.grade || '-', quantity: allocation.quantity }; }); return <button type="button" key={pkg.id} onClick={() => toggle(pkg.id)} className={`w-full rounded-xl border p-3 text-left transition ${checked ? 'border-emerald-600 bg-emerald-50' : 'border-slate-200 bg-white hover:border-emerald-300'}`}><div className="flex items-center justify-between gap-3"><span className="flex min-w-0 items-center gap-3"><span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-md border ${checked ? 'border-emerald-700 bg-emerald-700 text-white' : 'border-slate-300'}`}>{checked ? '✓' : ''}</span><span className="min-w-0"><b>Paket {pkg.letter}</b><span className="ml-2 text-sm text-slate-500">({pkg.items?.reduce((n, i) => n + i.quantity, 0) || 0} tanaman)</span></span></span><b className="shrink-0 text-slate-700">Rp {valueOf(pkg).toLocaleString('id-ID')}</b></div><div className="mt-2 border-t border-slate-200/80 pt-2 text-xs text-slate-600">{packageItems.map((item, index) => <p key={`${item.id}-${index}`}><span className="font-bold text-slate-800">{item.id}</span><span className="mx-1">•</span>Grade {item.grade}<span className="ml-1">×{item.quantity}</span></p>)}</div></button>; })}</div>{!packages.length && <p className="rounded-xl bg-amber-50 p-3 text-sm text-amber-800">Semua paket pada pesanan ini sudah diretur.</p>}</section>
        ...
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><div className="flex gap-2"><AlertTriangle className="h-5 w-5 shrink-0"/><div><p className="font-extrabold">Informasi</p><ul className="mt-1 list-disc space-y-0.5 pl-5"><li>Status pesanan akan berubah menjadi retur.</li><li>Omzet berkurang sesuai nominal refund (Rp {refund.toLocaleString('id-ID')}).</li><li>Komisi sales dipindahkan ke Pembayaran Ditolak.</li><li>Riwayat retur akan tersimpan di sistem.</li></ul></div></div></div>{error && <p className="rounded-xl bg-rose-50 p-3 text-sm text-rose-700">{error}</p>}
