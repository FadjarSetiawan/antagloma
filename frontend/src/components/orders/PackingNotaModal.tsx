  ...
  const modalContent = (
    <div id="packing-nota-modal-container" className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/40 backdrop-blur-xs p-3 sm:p-5 w-full h-full overflow-y-auto font-sans">
      <div className="bg-white rounded-2xl border border-slate-200 w-[95%] max-w-sm sm:max-w-md shadow-2xl overflow-hidden my-auto max-h-[92vh] flex flex-col font-sans">
        ...
        <div id="packing-nota-printable" className="p-3 sm:p-4 overflow-y-auto space-y-2.5 print-area text-slate-900 font-sans text-xs flex-1">
          ...
          <div className="border-b-2 border-black pb-2 pt-1 flex justify-between items-start gap-2">
            <div className="space-y-0.5 min-w-0 flex-1">
              <h1 className="font-bold text-[16px] tracking-normal text-black uppercase leading-normal pt-1 whitespace-nowrap">ANTAGLOMA FLORIST</h1>
            ...
            <div className="text-right shrink-0">
              ...
              <p className="font-bold text-sm text-black leading-tight tracking-tight whitespace-nowrap">{order.order_number}</p>
              <p className="text-[11.5px] font-normal text-black leading-tight">Tgl: {formattedDate}</p>
              {packageInfo && (
                <p className="text-xs font-bold text-black leading-tight mt-0.5">Paket {packageInfo.letter}</p>
              )}
            </div>
          </div>
          ...
          <div className="border-b border-black pb-2 space-y-1">
            <p className="text-xs font-bold text-black uppercase tracking-wider">PENERIMA / CUSTOMER:</p>
          ...
          <div className="border-b-2 border-black pb-2 space-y-0.5">
            <p className="text-xs font-bold text-black uppercase tracking-wider">ALAMAT PENGIRIMAN:</p>
            <p className="font-bold text-sm text-black leading-snug break-words">
              {[order.district_name, order.regency_name, order.province_name].filter(Boolean).join(', ')}
            </p>
            <p className="text-[12.5px] font-normal text-black leading-snug break-words">{order.full_address}</p>
          </div>
          ...
          <div className="border border-black rounded-lg p-2 space-y-1 bg-white my-1">
            <div className="flex items-center justify-between text-[11.5px] font-bold text-black uppercase tracking-wider">
            ...
            />
            <div className="hidden print:block thermal-print-only text-xs font-medium italic text-black whitespace-pre-wrap leading-tight">
              {customNotes.trim() ? customNotes : '-'}
            </div>
          </div>

          {/* Signatures Section */}
          <div className="pt-2 border-t border-black grid grid-cols-3 gap-1.5 text-center text-[11.5px] font-normal pb-3">
            <div>
              <span className="text-black font-semibold block mb-4">Sales</span>
              <span className="font-bold text-black block border-t border-black pt-0.5 leading-normal whitespace-nowrap">
        ...
        {/* Modal Footer Bar: Clean & Simple Action Buttons */}
