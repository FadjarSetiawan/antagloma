...
export const createPackingNotaCanvas = (data: PackingNotaCanvasData) => {
  ...
  const noteHeight = Math.max(142, 85 + noteLines.length * 29);
  ...
  const finalHeight = Math.min(WORKING_HEIGHT, Math.ceil(y + 26 + FOOTER_RESERVE));
  const croppedCanvas = document.createElement('canvas');
  croppedCanvas.width = WIDTH;
  croppedCanvas.height = finalHeight;
  const croppedContext = croppedCanvas.getContext('2d');
  if (!croppedContext) throw new Error('Gagal menyelesaikan bitmap nota.');
  croppedContext.drawImage(canvas, 0, 0, WIDTH, finalHeight, 0, 0, WIDTH, finalHeight);
